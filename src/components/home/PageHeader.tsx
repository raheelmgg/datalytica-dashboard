...
function PageHeader({ title, chips }: { title: string; chips?: boolean }) {
  ...
  return (
    <div>
      <div className="w-full max-w-full md:max-w-[58%] flex flex-col justify-start items-start md:flex-row md:justify-between gap-2">
            ...
            <Popover open={open} onOpenChange={setOpen}>
              <PopoverTrigger asChild>
                <div className="bg-white rounded-full p-1 flex flex-row justify-between items-center gap-2 cursor-pointer nowrap w-41.5">
                  ...
                  <ChevronDown className={`text-white h-5 w-5 bg-[#D9D9D9] rounded-full ${open ? "rotate-180" : ""}`} />
                </div>
              </PopoverTrigger>
